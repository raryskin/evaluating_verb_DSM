...
get_subdf_stdev <- function(dataset, string_Vector){
  share_summary <- dataset |> 
    group_by(.dots = string_Vector) |> 
    summarize(sd_sv_score = sd(sv_score))
  
  sh_df <- share_summary |>
    pivot_wider(names_from = all_of(string_Vector), values_from = sd_sv_score) |> 
    rename(shared = `TRUE`, not_shared = `FALSE`) |> 
    mutate(type = string_Vector)
...
for (ii in 1:length(frames_to_check)){
  ...
  # print(share_summary)
  if (nrow(share_summary) > 1){
    temp <- get_subdf_stdev(simverb, target_frame)
...
zoom2 <- ggplot(sh_df_sd, aes(x =shared, y = not_shared))+
  geom_point(shape = 21, fill = "lightblue")+
  # geom_label_repel(aes(x = shared, y = not_shared, label = type), 
  # box.padding = 0.5, max.overlaps = Inf)+
...
zoom_out2 <- ggplot(sh_df_sd, aes(x =shared, y = not_shared))+
  geom_point(shape = 21, fill = "lightblue")+
  # geom_label_repel(aes(x = shared, y = not_shared, label = type), 
  # box.padding = 0.5, max.overlaps = Inf)+
  geom_abline(slope = 1, intercept = 0, color = "darkgreen")+
  ...
  xlab("Shares a frame")+
  ylab("No frame shared")+
  ggtitle("StDev of similarity scores between verb pairs")

zoom_out2 + zoom2

basic_transitive_only <- simverb |> 
  filter(basic_transitive == T & basic_intransitive == F)

basic_intransitive_only <- simverb |> 
  filter(basic_transitive == F & basic_intransitive == T)

both <- simverb |> 
  filter(basic_transitive == T & basic_intransitive == T)

neither <- simverb |> 
  filter(basic_transitive == F & basic_intransitive == F)

test <- data.frame("share_type" = c("transitive_only",
                                    "intransitive_only",
                                    "both",
                                    "neither"),
                   "mean_sim" =c(mean(basic_transitive_only$sv_score),
                                 mean(basic_intransitive_only$sv_score),
                                 mean(both$sv_score),
                                 mean(neither$sv_score)))


ggplot(test, aes(x = factor(share_type, levels = c("transitive_only",
                                                   "intransitive_only",
                                                   "both",
                                                   "neither")), 
                            y = mean_sim, fill = share_type))+
  geom_bar(stat = "identity", color = "black")+
  scale_fill_manual(values = c("lightblue","lightgreen","brown","darkblue"))+
  theme(legend.position = "none")+
  xlab("Shared frame type")+
  ylab("Mean similarity")+
  ggtitle("Average similarity for different types of frame sharing")
